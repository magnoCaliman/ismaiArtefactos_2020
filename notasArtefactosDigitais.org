...
- A escolha de um sensor digital então se torna uma questão não de eletrônica ou de programação, mas de /design de interação/. O que é mais adequado para minha aplicação?
  - Detalhe para pensar: alguns sensores, como o de inclinação, possuem /interações analógicas/ (existem infinitas posições que o sensor pode estar) porém /repostas digitais/ (em algum momento sua leitura muda instantaneamente de ~HIGH~ para ~LOW~). 
